...
describe("WorksheetExercise", () => {
  it("renders a Japanese connected story with kanji stroke-order grids", () => {
    ...
    const { container } = render(
      <WorksheetExercise annotations={false} answers={false} exercise={exercise} index={0} language="ja" />,
    );
  ...
  it("renders a Burmese connected story with romanization annotations when enabled", () => {
    const exercise = readingExercise({
      passage: "ကျောင်းကို သွားမယ်။",
      annotations: [{ surface: "ကျောင်း", reading: "kyaung:" }],
    });
    const { container } = render(
      <WorksheetExercise annotations answers={false} exercise={exercise} index={0} language="my" />,
    );
    expect(container.querySelector("ruby")).toBeTruthy();
    expect(screen.getByText("kyaung:")).toBeTruthy();
  });
  ...
  it("renders a Burmese script-practice item in a plain practice grid (no stroke asset)", () => {
    ...
    const { container } = render(
      <WorksheetExercise annotations={false} answers={false} exercise={exercise} index={0} language="my" />,
    );
    expect(container.querySelector(".worksheet-stroke-hint")).toBeNull();
    expect(container.querySelector(".worksheet-model")).toBeTruthy();
  });

  it("renders a Polish connected story and an orthography drill answer key", () => {
    const story = readingExercise({ passage: "Pojechałam do Krakowa." });
    render(<WorksheetExercise annotations={false} answers={false} exercise={story} index={0} language="pl" />);
    ...
    render(<WorksheetExercise answers annotations={false} exercise={orthography} index={1} language="pl" />);
  ...
  it("falls back to an unsupported-type notice instead of silently rendering nothing", () => {
    ...
    render(<WorksheetExercise annotations={false} answers={false} exercise={exercise} index={0} language="ja" />);
